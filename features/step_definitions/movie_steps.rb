...
Then(/^I should see "(.*)" before "(.*)" in the movie list$/) do |e1, e2|
  ...
  page_body = page.body
  movie_1 = page_body.index(e1)
  movie_2 = page_body.index(e2)
  expect(movie_1).to be < movie_2
end
...
When(/I check the following ratings: (.*)/) do |rating_list|
  ...
  all_rating = %w[G PG PG-13 R]
  selected_rating = rating_list.split(/,\s*/)

  all_rating.each do |rating|
    if selected_rating.include?(rating)
      step %{I check "#{rating}"}
    else
